...
def discord_update_loop():
    while True:
        try:
            elapsed = int(time.time() - start_time)
            rpc.update(
                state=f"Clicks: {click_count.get()}",
                details=f"Petting Niko",
                start=start_time,
                large_image="nikosprite",
                large_text="niko speak"
            )
        except Exception as e:
...
click_count_label = tk.Label(control_frame, text="Clicks: 0", fg="#f0f0f0", bg="#2a2a2a", font=("Segoe UI", 12, "bold"))
click_count_label.grid(row=0, column=0, padx=5)

threading.Thread(target=discord_update_loop, daemon=True).start()
...
def on_button_click():
    ...
    if chance == 1:
        ...
        def reset():
            if rare_sound1: rare_sound1.play(); time.sleep(rare_sound1.get_length())
            change_sprite(niko_normal_img)
        threading.Thread(target=reset, daemon=True).start()
    elif chance == 2:
        change_sprite(niko_rare2_img)
        def reset():
            if rare_sound2: rare_sound2.play(); time.sleep(rare_sound2.get_length())
            change_sprite(niko_normal_img)
        threading.Thread(target=reset, daemon=True).start()
    else:
        ...
        def reset():
            if normal_sound: normal_sound.play(); time.sleep(normal_sound.get_length())
            change_sprite(niko_normal_img)
        threading.Thread(target=reset, daemon=True).start()
...
volume_slider = tk.Scale(control_frame, from_=0.0, to=1.0, orient="horizontal", resolution=0.01,
    length=150, label="Volume", command=set_volume, bg="#2a2a2a", fg="white",
    troughcolor="#444444", highlightthickness=0, font=("Segoe UI", 10, "bold"), showvalue=0)
volume_slider.set(default_music_volume)
volume_slider.grid(row=0, column=3, padx=5)

volume_percent_label = tk.Label(control_frame, textvariable=volume_percent_var,
    fg="white", bg="#2a2a2a", font=("Segoe UI", 10, "bold"))
